...
update_gamma_additive <- function(params){
  ...
  for(j in sample(1:p)){ # randomize order
    ...
    # V0 <- matrix(0,nrow=length(which0),ncol=length(which0)) ## Variance of correct size
    # V1 <- matrix(0,nrow=length(which1),ncol=length(which1)) ## Variance of correct size
    # for(k in 1:p){
    #   V0 <- V0+(gamma0[k]/tau2_additive[k])*Sstar[[k]][which0,which0]
    #   V1 <- V1+(gamma1[k]/tau2_additive[k])*Sstar[[k]][which1,which1]
    # }
    if(length(which0)==0){ ## error handling in case there are no components left
      # chol1 <- chol(as.matrix(t(B1)%*%invSIG%*%B1+V1))
      chol1 <- chol(as.matrix(eigenQuadProd(invSIG,B1)+V1))# as.matrix because otherwise chol gives an error about non-symmetry

      W1 <- chol2inv(chol1)

      logRratio <- 0.5*(d[j]-1)*log(tau2_additive[j])-0.5*log(det(as.matrix((Sstar[which1,which1])[-length(which1),-length(which1)])))+ ## *****ADDED d[j]-1 and -length(which1) to remove unpenalized terms
        # # old version2: -0.5*log(det(Sstar[which1,which1]/tau2_additive[j]))+ ## the which1-which0 just selects the jth components, thus gives Sj
        # # old version separated the tau2: # 0.5*d[j]*log(tau2_additive[j])-0.5*log(det(Sstar[which1,which1]))
         -0.5*(0-2*sum(log(diag(chol1))))+
        0.5*t(invSIGmu)%*%( - eigenQuadProd(W1,B1) )%*%invSIGmu #0.5*t(invSIGmu)%*%( - B1%*%W1%*%t(B1) )%*%invSIGmu

    }else{

      chol0 <- chol(as.matrix( eigenQuadProd(invSIG,B0)+V0))#chol(as.matrix(t(B0)%*%invSIG%*%B0+V0))# as.matrix because otherwise chol gives an error about non-symmetry
      chol1 <- chol(as.matrix(eigenQuadProd(invSIG,B1)+V1))#chol1 <- chol(as.matrix(t(B1)%*%invSIG%*%B1+V1))# as.matrix because otherwise chol gives an error about non-symmetry
      ...
      logRratio <- 0.5*(d[j]-1)*log(tau2_additive[j])-0.5*log(det(as.matrix((Sstar[which1[!(which1%in%which0)],which1[!(which1%in%which0)]])[-length(which1[!(which1%in%which0)]),-length(which1[!(which1%in%which0)])])))+ ## *****ADDED d[j]-1 and -length(which1) to remove unpenalized terms
        # # old version2:-0.5*log(det(Sstar[which1[!(which1%in%which0)],which1[!(which1%in%which0)]]/tau2_additive[j]))+ ## the which1[!(which1%in%which0)] just selects the jth components, thus gives Sj
        # # old version separated the tau2: # 0.5*d[j]*log(tau2_additive[j])-0.5*log(det(Sstar[which1[!(which1%in%which0)],which1[!(which1%in%which0)]]))+ ## the which1[!(which1%in%which0)] just selects the jth components, thus gives Sj
        -0.5*(2*sum(log(diag(chol0)))-2*sum(log(diag(chol1))))+
        # 0.5*t(invSIGmu)%*%( B0%*%W0%*%t(B0) - B1%*%W1%*%t(B1)  )%*%invSIGmu  ## swapped the transposes in line with the correct dimensions here
        0.5*t(invSIGmu)%*%( eigenQuadProd(W0,t(B0)) - eigenQuadProd(W1,t(B1))  )%*%invSIGmu  ## swapped the transposes in line with the correct dimensions here
      # old version had B1 before B0: #0.5*t(invSIGmu)%*%( B0%*%W0%*%t(B0) - B1%*%W1%*%t(B1)  )%*%invSIGmu  ## swapped the transposes in line with the correct dimensions here
...
update_beta_additive <- function(params){
  ...
  if(sum(gamma_additive)>0){
    ...
    # Vgam <- matrix(0,nrow=length(whichcols),ncol=length(whichcols)) ## Variance of correct size
    # for(j in 1:p){
    #   Vgam <- Vgam+(gamma_additive[j]/tau2_additive[j])*Sstar[[j]][whichcols,whichcols]
    # }
    # EDIT: # V <- as.matrix(solve(eigenQuadProd(invSIG,Bgam)+Vgam))#as.matrix(solve(t(Bgam)%*%invSIG%*%Bgam+Vgam)) ## convert to matrix so that mvtnorm works
    V <- as.matrix(FastGP::rcppeigen_invert_matrix(as.matrix(eigenQuadProd(invSIG,Bgam)+Vgam)))#as.matrix(solve(t(Bgam)%*%invSIG%*%Bgam+Vgam)) ## convert to matrix so that mvtnorm works
...
#   gamma_additive <- params$params$gamma_additive
#
#   new_tau2_additive <- rep(NA,p)
#   for(j in 1:p){
#     whichx <- rep(0,p)
#     whichx[j] <- 1
#     whichcols <- which(rep(whichx,d)==1)
#     new_tau2_additive[j] <- 1/rgamma(1,
#                                      shape=a_tau+gamma_additive[j]*d[j]/2,
#                                      rate=b_tau+as.numeric(gamma_additive[j]*t(beta[whichcols])%*%Sstar[whichcols,whichcols]%*%beta[whichcols])/2)
#   }
#   newparams$params$tau2_additive <- new_tau2_additive
